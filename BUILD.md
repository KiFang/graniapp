...
Все команды — в PowerShell, в папке проекта. Отдельно ставить `eas` не нужно — скрипты запускают его через `npx`.
...
3. **Подключить бота.** Откройте в браузере
   ...
   Команды бота: `/qr` (Player ID), `/myevents` (мои записи, кнопка «Не смогу»), `/friends`, `/stats`, `/help`.
   После обновления бота откройте `?setup=1` ещё раз — так Telegram узнает о новых командах и кнопках.
